 ...
 * Guest: creates guest_user → booking with guest_user_id (THB only)
 * Registered: creates booking with customer_id (profile.id)
 */
...
const BookingSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Price is 0 initially — admin will set actual price from zone pricing
      // booking_number is generated by DB trigger, but we provide a fallback
      const fallbackBookingNumber = 'IBB-' + new Date().toISOString().slice(0,10).replace(/-/g,'') + '-' + Math.floor(Math.random() * 10000).toString().padStart(4, '0');
      ...
      const bookingData = {
        ...
        guest_email: guestEmail || formData.email,
        pickup_location: formData.pickupLocation,
        dropoff_location: formData.dropoffLocation,
        pickup_datetime: pickupDatetime,
        vehicle_type: formData.vehicleType as "car" | "van" | "bus",
        ...
        final_price: 0,
        currency: "THB",
        booking_number: fallbackBookingNumber,
      };

      const { data: booking, error } = await supabase
        .from("bookings")
        .insert(bookingData)
        .select("booking_number, final_price, currency")
        .single();

      if (error) {
        console.error("Booking creation error:", error.message, error.details, error.hint);
